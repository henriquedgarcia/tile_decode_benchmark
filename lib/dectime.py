...
class UserDectime:
    ...
    class GetTiles(Worker, GetTilesPath):
        results: AutoDict
        database: AutoDict
        old_video = None
        old_tiling = None
    ...
    class ViewportMetrics(Worker, GetTilesPath):
        def __init__(self, config):
            self.config = config
            self.print_resume()
            self.extract_viewport()

        def extract_viewport(self, overwrite=False):
            ...
            for self.video in self.videos_list:
                ...
                nfov = NFOV(proj_w, proj_h, FOV=(110, 90))
                        ...
                        for self.quality in quality_list:
                            ...
                            for self.chunk in self.chunk_list:
                                ...
                                for _ in range(30):  # 30 frames per chunk
                                    ...
                                    start = time.time()
                                    fov_ref = nfov.get_viewport(img_proj, center_point)  #.astype('float64')
