...
class GlobalPaths(Factors):
    ...
    def dataset_folder(self) -> Path:
        folder = Path('datasets') / self.config['dataset_name']
...
class QualityAssessmentPaths(TileDecodeBenchmarkPaths):
    ...
    @property
    def reference_file(self) -> Union[Path, None]:
...
class DectimeGraphs:
    def __init__(self, config: str, role: str):
        operations = {'BY_PATTERN': self.ByPattern,
                      ...
                      'BY_VIDEO_BY_PATTERN_BY_TILE_BY_CHUNK': self.ByVideoByPatternByTileByChunk,
            ...
            for self.metric in self.metric_list:
                ...
                for self.video in self.videos_list:
                    ...
                    for self.tiling in self.tiling_list:
                        ...
                        samples = data_bucket[self.vid_proj][self.tiling]
                            ...
                            for self.tile in self.tile_list:
                                for self.chunk in self.chunk_list:
                                    ...
                                    try:
                                        ...
                                        samples = data_bucket[self.vid_proj][self.tiling] = [value]
            ...
            for self.metric in self.metric_list:
                ...
                for self.proj in self.proj_list:
                    for self.tiling in self.tiling_list:
                        ...
                        # Get samples
                        try:
                            samples = data_bucket[self.proj][self.tiling]
                        except TypeError:
    ...
    class ByVideoByPatternByTileByChunk(Worker, DectimeGraphsPaths):
        ''' Série temporal
    ...
    #
    #                     # Load data file
    #                     samples = self.data_bucket[self.proj][self.tiling][self.metric]
    #
    #                     # Make the fit
    #                     distributions = self.config['distributions']
    #                     fitter = Fitter(samples, bins=self.bins, distributions=distributions, timeout=900)
    ...
    #             for self.tiling in self.tiling_list:
    #                 # Load data
    #                 data_bucket = self.data_bucket[self.proj][self.tiling]
    #
    #                 for self.metric in self.metric_list:
    #                     # Load fitter pickle
    ...
    #                 data_by_tiling = [self.data_bucket[self.proj][self.tiling][self.metric] for self.tiling in self.tiling_list]
    #
    #                 ################ joined axes ################
    ...
    #
    #                 for self.proj in self.proj_list:
    #                     tiling_data = self.data_bucket[self.proj][self.tiling][self.metric]
    ...
    #                 ax_sep: axes.Axes = fig_boxplot_separated.add_subplot(nrows_sep, ncols_sep, index_sep)
    #
    #                 tiling_data = [self.data_bucket[self.proj][self.tiling][self.metric] for self.proj in self.proj_list]
