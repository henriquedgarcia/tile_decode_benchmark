...
from typing import Any, NamedTuple, Union, Dict, List, Optional, Generator
...
class Factors:
    ...
    def proj_list(self) -> list[str]:
        projs = set([self.videos_list[self.video]['projection'] for video in self.videos_list])
        return list(projs)
...
class GlobalPaths(Factors):
    ...
    def dataset_json(self) -> Path:
        ...
        database_json = self.database_folder / f'{self.config["dataset_name"]}.json'
...
class Worker(GlobalPaths):
    ...
    def count_decoding(self, dectime_log) -> int:
        """
        Count how many times the word "utime" appears in "log_file"
        :return:
        """
    ...
    def run_command(self, command: str, log_file: Union[str, Path] = None, mode: str = None):
        """
...
class DectimeGraphs:
    def __init__(self, config: str, role: str):
        operations = {'BY_PATTERN': self.ByPattern,
                      'BY_PATTERN_BY_QUALITY': self.ByPattern,
    ...
    class ByPattern(Worker, DectimeGraphsPaths):
        ...
        def workfolder(self) -> Path:
            folder = self.project_path / self.graphs_folder / 'BY_PATTERN'
            folder.mkdir(parents=True, exist_ok=True)
            return folder
        ...
        def video_context_get_data(self):
            for _ in range(1):
                yield

        def __init__(self, config):
            self.video_context = self.video_context_get_data
            self.work = self.get_get_tiles_data
            super().__init__(config)
            # self.work = self.get_data_bucket
            # super().__init__(config)
        ...
        def get_data_bucket(self, overwrite=False, remove_outliers=False):
            ...
            if self.data_bucket_file.exists() and not overwrite:
                warning(f'  The data file "{self.data_bucket_file}" exist. Skipping.')
                return
            ...
            for self.video in self.videos_list:
                ...
                for self.tiling in self.tiling_list:
                    bucket = self.data_bucket[self.vid_proj][self.tiling]
                    print(f'\r  Dectime - {self.vid_proj}  {self.name} {self.tiling} ... ', end='')
                    ...
                    for self.quality in self.quality_list:
                        for self.tile in self.tile_list:
                            for self.chunk in self.chunk_list:
                                ...
                                try:
                                    bucket['time'].append(float(np.average(dectime_val['times'])))
                                except AttributeError:
                                    bucket = self.data_bucket[self.vid_proj][self.tiling] = defaultdict(list)
                                    bucket['time'].append(float(np.average(dectime_val['times'])))
                                bucket['time_std'].append(float(np.std(dectime_val['times'])))
                                bucket['rate'].append(float(bitrate_val['rate']))
            ...
            if remove_outliers:
                self.remove_outliers(self.data_bucket)

            print(f'  Saving dectime... ', end='')
            save_json(self.data_bucket, self.data_bucket_file)
            print(f'  Finished.')

        def work_(self, overwrite=True):

            self.make_fit(overwrite)
            self.calc_stats(overwrite)

            DectimeGraphs.rc_config()
            self.make_hist(overwrite)
            self.make_bar(overwrite)
            self.make_boxplot(overwrite)
            self.make_violinplot(overwrite)
        ...
        def make_fit(self, overwrite=False):
            ...
            if self.data is None:
                self.data = load_json(self.data_bucket_file, object_hook=dict)
            ...
            for self.proj in self.proj_list:
                for self.tiling in self.tiling_list:
                    for self.metric in self.metric_list:
                        ...
                        # Check fitter pickle
                        if self.fitter_pickle_file.exists() and not overwrite:
        ...
        def calc_stats(self, overwrite=False):
            ...
            pd.DataFrame(stats).to_csv(str(self.stats_file), index=False)
            pd.DataFrame(corretations_bucket).to_csv(str(self.correlations_file), index=False)

        @property
        def hist_pattern_file(self) -> Path:
            folder = self.workfolder / 'hist_pattern'
            file = folder / f'hist_pattern_{self.proj}_{self.metric}_{self.bins}bins.png'
            return file
        ...
        def remove_outliers(self, data, resumenamecsv=None):
        ...
        def make_hist(self, overwrite=False):
            ...
            for self.metric in self.metric_list:
                for self.proj in self.proj_list:
                    ...
                    im_file = self.hist_pattern_file
        ...
        def make_bar(self, overwrite=False):
        ...
        def make_boxplot(self, overwrite=False):
            ...
            # Check image file by metric
            for self.metric in self.metric_list:
                img_file = self.boxplot_pattern_file
...
#
#
# class QualityAssessmentPaths(Factors, TileDecodeBenchmarkPaths):
#     @property
...
#
#
# class QualityAssessment(QualityMetrics):
#     results: AutoDict
#     old_video = ''
#
...
#         :param kwargs: passed to main routine
#         """
#         operations = {'PREPARE': self.process_nasrabadi,
#                       'GET_TILES': self.get_tiles,
#                       'USER_ANALISYS': self.user_analisys,
#                       }
#
#         self.config = Config(config)
#         self.role = operations[role]
#         self.video_context = VideoContext(self.config, 0)
#
